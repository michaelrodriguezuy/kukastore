...
import { Box, Container, Grid, Typography, Button, Divider, Paper, FormControl, InputLabel, Select, MenuItem } from '@mui/material';
...
const CartPage = () => {
  ...
  const handleCalculateShipping = async () => {
    try {
      ...
      if (shippingMethod === 'envio' && shippingType === 'ues') {
        ...
        const addressParts = shippingData.address ? shippingData.address.split(' ') : ['', ''];
        const streetNumber = addressParts[addressParts.length - 1].match(/\d+/) ? addressParts.pop() : '';
        ...
        const shippingRequest = {
          ...
          Calle: uesDeliveryType === 'agencia' ? agenciaData.direccion.calle : streetName,
          Nro: uesDeliveryType === 'agencia' ? agenciaData.direccion.nro : streetNumber,
          CodPo: uesDeliveryType === 'agencia' ? agenciaData.direccion.codPo : (shippingData.postalCode || '00000'),
        ...
        try {
          const response = await axios.post(
            ...
            'https://8rx6nnr9-8081.brs.devtunnels.ms/api/shipping/calculate', 
            shippingRequest
          );
          ...
          const shippingData = {
            ...
            shippingMethod: uesDeliveryType === 'agencia' ? 'UES Agencia' : 'UES Domicilio',
            shippingCost: 0,
      ...
      if (shippingMethod === 'envio' && shippingType === 'agencia') {
        ...
        const shippingDataToSave = {
          shippingMethod: otherDeliveryType === 'agencia' ? 
        ...
        localStorage.setItem('shippingData', JSON.stringify(shippingDataToSave));
        
        // Actualizar el costo en el contexto del carrito
        updateOrderData({
      ...
      if (shippingMethod === 'retiro') {
        ...
        setShippingCost(cost);
        setShippingCalculated(true);
        if (typeof updateShipmentCost === 'function') {
          updateShipmentCost(cost);
        }
  ...
  const isFormValid = () => {
    if (shippingMethod === 'retiro') return true;
      ...
      if (shippingType === 'ues') {
        ...
        if (uesDeliveryType === 'domicilio' && !shippingData.address) return false;
      ...
      if (shippingType === 'agencia') {
        ...
        if (otherDeliveryType === 'domicilio' && !shippingData.address) return false;
        if (otherDeliveryType === 'agencia' && !shippingData.agencyName) return false;
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        // const response = await axios.get('http://localhost:8081/api/ues/locations');
        const response = await axios.get('https://8rx6nnr9-8081.brs.devtunnels.ms/api/ues/locations');
  ...
  useEffect(() => {
    const checkAgenciaDisponible = async () => {
      if (uesDeliveryType === 'agencia' && selectedDepartamento && selectedLocalidad) {
        try {
          const response = await axios.get(
            // `http://localhost:8081/api/ues/agencia/${selectedDepartamento}/${selectedLocalidad}`
            `https://8rx6nnr9-8081.brs.devtunnels.ms/api/ues/agencia/${selectedDepartamento}/${selectedLocalidad}`
          );
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={4}>
            <Paper sx={{ p: 3, bgcolor: 'rgba(0, 0, 0, 0.02)', borderRadius: 2 }}>
              <Typography variant="h6" fontWeight="bold" align="center" sx={{ mb: 3 }}>Total del carrito</Typography>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography>Subtotal</Typography>
                <Typography>{getFormatCurrency(cart.reduce((total, item) => total + (item.unit_price * item.quantity), 0))}</Typography>
              </Box>

              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography>Descuento</Typography>
                <Typography>{getFormatCurrency(cart.reduce((total, item) => {
                  if (item.discount) {
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
                ...
                <Typography variant="h6">{getFormatCurrency(getTotalPrice())}</Typography>
              </Box>
            </Paper>

            <Paper sx={{ p: 3, mt: 3, bgcolor: 'rgba(0, 0, 0, 0.02)', borderRadius: 2 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>Envío</Typography>
              ...
              {shippingMethod === 'envio' && (shippingType === 'ues' || shippingType === 'agencia') && (
                <Button
                  variant="contained"
                  fullWidth
